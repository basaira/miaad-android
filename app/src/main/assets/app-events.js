...
['fDay','fDuration','fStart'].forEach(id=>document.getElementById(id).addEventListener('input',renderAvailability));
...
document.getElementById('saveLesson').onclick=()=>{
 ...
 const duration=Math.max(0,+document.getElementById('fDuration').value||0),
   ...
   obj={
     ...
     start:document.getElementById('fStart').value||'17:00',duration,
 ...
 const check=SmartScheduleEngine.conflict(chosenSheetDate(),toSeconds(obj.start),obj.duration||30,oldId);

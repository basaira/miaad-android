...
document.getElementById('importBtn').onclick=()=>document.getElementById('importFile').click();document.getElementById('importFile').onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{const data=JSON.parse(await file.text());if(!data||!Array.isArray(data.lessons)||data.lessons.some(l=>!l||typeof l.id!=='string'||typeof l.name!=='string'||!Number.isInteger(l.day)||l.day<0||l.day>6))throw new Error('صيغة غير صحيحة');const modernImport=isModernDomainPayload(data.domain);domain.merge(modernImport?data.domain:undefined);const existing=new Map(lessons.map(x=>[x.id,x]));data.lessons.forEach(x=>existing.set(x.id,x));lessons=normalizeSchedule([...existing.values()]);sessionState={...sessionState,...(data.sessionState||{})};sessionNotes={...sessionNotes,...(data.sessionNotes||{})};for(const [k,v] of Object.entries(data.sessionAudit||{}))sessionAudit[k]=[...(sessionAudit[k]||[]),...v].sort((a,b)=>String(a.at).localeCompare(String(b.at)));if(Number.isFinite(Number(data.idrisPhase)))idrisPhase=Number(data.idrisPhase);domain.replace(domain.data,{}, {migrateLegacy:false});if(!modernImport){const legacyKeys=new Set([...Object.keys(data.sessionState||{}),...Object.keys(data.sessionNotes||{})]),groups=new Map();for(const l of data.lessons){if(!groups.has(l.name))groups.set(l.name,[]);groups.get(l.name).push(l)}for(const [name,group] of groups){const scheduleIds=new Set(group.map(l=>l.id)),candidateDates=[];for(const key of legacyKeys){const marker=key.indexOf('__');if(marker<1)continue;const D=key.slice(0,marker),scheduleId=key.slice(marker+2);if(scheduleIds.has(scheduleId)&&Number.isInteger(domain.weekdayForDate(D)))candidateDates.push(D)}candidateDates.sort();const existingStudent=Object.values(domain.data.students).find(s=>s.name===name),student=domain.legacyStudentFor(name,candidateDates[0]||'');for(const l of group)domain.importLegacySchedule({...l,studentId:student.id});if(!existingStudent){const validDates=[];for(const key of legacyKeys){const marker=key.indexOf('__');if(marker<1)continue;const D=key.slice(0,marker),scheduleId=key.slice(marker+2);if(scheduleIds.has(scheduleId)&&domain.resolveScheduleOccurrence(scheduleId,D))validDates.push(D)}validDates.sort();const resolvedStart=validDates[0]||domain.teacherNow().date;if(student.startDate!==resolvedStart)domain.saveStudent({...student,startDate:resolvedStart})}}for(const key of legacyKeys){if(domain.data.records[key])continue;const marker=key.indexOf('__');if(marker<1)continue;const D=key.slice(0,marker),scheduleId=key.slice(marker+2),projection=domain.resolveScheduleOccurrence(scheduleId,D);if(!projection)continue;domain.record({...projection,status:data.sessionState?.[key]||'pending',note:data.sessionNotes?.[key]||''})}}refreshLegacyOccurrenceMirrors();persist();renderAll();showToast('تم دمج قاعدة البيانات بنجاح')}catch(err){showToast('تعذر استيراد الملف: صيغة JSON غير متوافقة')}finally{e.target.value=''}};

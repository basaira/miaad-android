...
check('AM Phase 1C teacher civil occurrence date ignores device zone',()=>{const d=createMiaadDomain({settings:{teacherTimeZone:'Asia/Tokyo'}},{},()=>new Date('2026-09-30T16:30:00Z')),s=d.saveStudent({id:'am',name:'AM',startDate:'2026-09-01',custom:false,settings:{}});d.saveSchedule({id:'am-occ',studentId:s.id,name:s.name,day:4,start:'09:00',duration:30,repeat:'weekly'},'2026-10-01');assert.equal(d.teacherNow().date,'2026-10-01');assert.ok(d.resolveScheduleOccurrence('am-occ','2026-10-01'));assert.equal(d.resolveScheduleOccurrence('am-occ','2026-09-30'),null)});
check('AN Phase 1C DST-adjacent record identity remains civil',()=>{const d=createMiaadDomain({settings:{teacherTimeZone:'America/New_York'}},{},()=>new Date('2026-03-08T12:00:00Z')),s=d.saveStudent({id:'an',name:'AN',startDate:'2026-01-01',custom:false,settings:{}});d.saveSchedule({id:'dst-occ',studentId:s.id,name:s.name,day:0,start:'02:30',duration:30,repeat:'weekly'},'2026-03-08');const r=d.resolveScheduleOccurrence('dst-occ','2026-03-08');assert.ok(r);assert.equal(r.id,'2026-03-08__dst-occ');assert.equal(d.resolveWallClock(r.date,r.time).toISOString(),'2026-03-08T07:30:00.000Z')});

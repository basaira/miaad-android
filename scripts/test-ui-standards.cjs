...
(async()=>{
 ...
 const report={standardBasis:{
   ...
   compactCalendar:'At 320 CSS px, seven simultaneous 48px columns require 336px before any outer spacing. To preserve a fully visible no-horizontal-scroll month grid, compact-320 month-day cells below 48px are recorded as Android advisories, not Android passes. WCAG AA 24px, keyboard access and no-overflow remain mandatory.'
 },viewports:{}};
 const failures=[];
 for(const [name,width,height] of viewports){
  ...
  for(const view of views){
    ...
    const compactCalendarAdvisory=targets.filter(t=>width<336&&t.className.split(/\s+/).includes('month-day')&&(t.width<48||t.height<48));
    ...
    if(view==='today'){
      // :focus-visible is intentionally modality-sensitive. Exercise the UI with
      // a real keyboard Tab rather than programmatic element.focus(), which does
      // not reliably activate the focus-visible heuristic in Chromium.
      await page.evaluate(()=>{if(document.activeElement instanceof HTMLElement)document.activeElement.blur()});
      await page.keyboard.press('Tab');
      focusAppearance=await page.evaluate(()=>{const e=document.activeElement,s=e?getComputedStyle(e):null;return {tag:e?.tagName||'',id:e?.id||'',className:String(e?.className||''),outlineStyle:s?.outlineStyle||'',outlineWidth:parseFloat(s?.outlineWidth)||0,outlineColor:s?.outlineColor||''}});
      if(!focusAppearance.tag||focusAppearance.outlineStyle==='none'||focusAppearance.outlineWidth<2)failures.push(`${name}/${view}: visible keyboard focus ring missing`);
    }
 ...
 console.log('UI STANDARDS PASS: WCAG 2.2 AA axe rules and target size, Android 48dp guidance with documented compact-320 calendar advisories, real keyboard focus, RTL, responsive overflow and fixed-nav safety across six viewport classes.');

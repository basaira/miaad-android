...
approved = Image.open(BytesIO(data)).convert("RGB")
if approved.width < MIN_MASTER_PX or approved.height < MIN_MASTER_PX:
    raise SystemExit(f"Master logo resolution is too small: {approved.size}")

# Normalize once to a 512px production master. 384px tracked source remains
# comfortably above the 192px xxxhdpi launcher target and preserves the approved artwork.
master = approved.resize((512,512), Image.Resampling.LANCZOS)
MASTER.parent.mkdir(parents=True, exist_ok=True)
master.save(MASTER, "WEBP", quality=98, method=6)

ASSETS.mkdir(parents=True, exist_ok=True)
master.save(ASSETS / "miaad-logo.webp", "WEBP", quality=98, method=6)
...
master.save(nodpi / "miaad_logo.webp", "WEBP", quality=98, method=6)

fg = Image.new("RGBA", (432,432), (0,0,0,0))
inner = master.resize((338,338), Image.Resampling.LANCZOS).convert("RGBA")
fg.alpha_composite(inner, ((432-338)//2,(432-338)//2))
fg.save(nodpi / "ic_launcher_foreground.webp", "WEBP", quality=98, method=6)
...
print(f"MIAAD approved branding materialized from {source}: source={approved.width}x{approved.height}, production=512x512")
